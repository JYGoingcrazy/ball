...
// 设置SDA为输入模式
#define SDA_IN()	gpio_mode_set(I2C0_SDA_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, I2C0_SDA_PIN);
// 设置SDA为输出模式
#define SDA_OUT()	gpio_mode_set(I2C0_SDA_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, I2C0_SDA_PIN);
// 读取SDA电平
...
int8_t I2C0_write(uint8_t addr, uint8_t reg, uint8_t* data, uint32_t len){
	return I2C0_write2(addr, reg, data, 1, len);
}
